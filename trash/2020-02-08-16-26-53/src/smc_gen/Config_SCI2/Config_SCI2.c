...
***********************************************************************************************************************/
volatile uint8_t   g_sci2_iic_transmit_receive_flag; /* SCI2 transmit receive flag for I2C */
volatile uint8_t   g_sci2_iic_cycle_flag;            /* SCI2 start stop flag for I2C */
volatile uint8_t   g_sci2_slave_address;             /* SCI2 target slave address */
...
void R_Config_SCI2_Create(void)
{
    ...
    SCI2.SIMR3.BYTE = _C0_SCI_SSCL_HIGH_IMPEDANCE | _30_SCI_SSDA_HIGH_IMPEDANCE;

    /* Set up transfer or reception format in SMR and SCMR */
    SCI2.SMR.BYTE = _00_SCI_CLOCK_PCLK | _00_SCI_ASYNCHRONOUS_OR_I2C_MODE;
    ...
    /* Set bit rate */
    SCI2.BRR = 0x04U;
    SCI2.SEMR.BYTE = _00_SCI_NOISE_FILTER_DISABLE | _00_SCI_BIT_MODULATION_DISABLE;
    SCI2.SIMR1.BYTE |= (_01_SCI_IIC_MODE | _00_SCI_NONE);
    SCI2.SIMR2.BYTE |= (_00_SCI_ACK_NACK_INTERRUPTS | _02_SCI_SYNCHRONIZATION | _20_SCI_NACK_TRANSMISSION);
    SCI2.SPMR.BYTE = _00_SCI_CLOCK_NOT_INVERTED | _00_SCI_CLOCK_NOT_DELAYED;
    SCI2.SCR.BYTE = _10_SCI_RECEIVE_ENABLE | _20_SCI_TRANSMIT_ENABLE | _40_SCI_RXI_ERI_ENABLE | _80_SCI_TXI_ENABLE | 
                    _04_SCI_TEI_INTERRUPT_ENABLE;

    /* Set SSCL2 pin */
    MPC.P12PFS.BYTE = 0x0AU;
    PORT1.ODR0.BYTE |= 0x10U;
    PORT1.PMR.BYTE |= 0x04U;

    /* Set SSDA2 pin */
    MPC.P13PFS.BYTE = 0x0AU;
    PORT1.ODR0.BYTE |= 0x40U;
...
* Description  : This function stops the SCI2 channel
* Arguments    : None
* Return Value : None
***********************************************************************************************************************/
...
/***********************************************************************************************************************
* Function Name: R_Config_SCI2_IIC_StartCondition
* Description  : This function generates IIC start condition
* Arguments    : None
* Return Value : None
***********************************************************************************************************************/

void R_Config_SCI2_IIC_StartCondition(void)
{
    SCI2.SIMR3.BYTE = _01_SCI_START_CONDITION_ON | _10_SCI_SSDA_START_RESTART_STOP_CONDITION | 
                      _40_SCI_SSCL_START_RESTART_STOP_CONDITION;
}

/***********************************************************************************************************************
* Function Name: R_Config_SCI2_IIC_StopCondition
* Description  : This function generates IIC stop condition
* Arguments    : None
* Return Value : None
***********************************************************************************************************************/

void R_Config_SCI2_IIC_StopCondition(void)
{
    SCI2.SIMR3.BYTE = _04_SCI_STOP_CONDITION_ON | _10_SCI_SSDA_START_RESTART_STOP_CONDITION | 
                      _40_SCI_SSCL_START_RESTART_STOP_CONDITION;
}

/***********************************************************************************************************************
* Function Name: R_Config_SCI2_IIC_Master_Send
* Description  : This function sends simple IIC(SCI2) data to slave device
* Arguments    : adr -
...
void R_Config_SCI2_IIC_Master_Send(uint8_t adr, uint8_t * const tx_buf, uint16_t tx_num)
{
    if (tx_num < 1U)
    {
        return;
    ...
    g_sci2_slave_address = adr;
    g_sci2_iic_transmit_receive_flag = _80_SCI_IIC_TRANSMISSION;
    g_sci2_iic_cycle_flag = _80_SCI_IIC_START_CYCLE;

    /* Generate start condition */
    R_Config_SCI2_IIC_StartCondition();
}

/***********************************************************************************************************************
* Function Name: R_Config_SCI2_IIC_Master_Receive
* Description  : This function receives simple IIC(SCI2) data from slave device
* Arguments    : adr -
*                    slave device address
*                rx_buf -
...
void R_Config_SCI2_IIC_Master_Receive(uint8_t adr, uint8_t * const rx_buf, uint16_t rx_num)
{
    if (rx_num < 1U)
    {
        return;
    ...
    g_sci2_slave_address = adr;
    g_sci2_iic_transmit_receive_flag = _00_SCI_IIC_RECEPTION;
    g_sci2_iic_cycle_flag = _80_SCI_IIC_START_CYCLE;
